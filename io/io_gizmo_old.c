...
void gizmo_old_readmass_data(hid_t HDF_FileID, char *filename, float *massTable) {
    ...
    massTable[OGIZM_DM_PARTTYPE] = buffer[0];
}
...
void gizmo_old_rescale_particles(struct particle *p, int64_t p_start, int64_t nelems) {
  ...
  for (int64_t i=0; i<nelems; i++) {
    for (int64_t j=0; j<3; j++) {
      p[p_start+i].pos[j]   *= OGIZM_LENGTH_CONVERSION;
...
{	
  ...
  Om = gizmo_old_readheader_float(HDF_Header, filename, "Omega0");          uint32_t npart_low[OGIZM_NTYPES], npart_high[OGIZM_NTYPES] = {0};  
  h0 = gizmo_old_readheader_float(HDF_Header, filename, "HubbleParam");     int64_t npart[OGIZM_NTYPES];
  SCALE_NOW = gizmo_old_readheader_float(HDF_Header, filename, "Time");     float massTable[OGIZM_NTYPES];
  BOX_SIZE = gizmo_old_readheader_float(HDF_Header, filename, "BoxSize");
  BOX_SIZE *= OGIZM_LENGTH_CONVERSION;      gizmo_old_readheader_array(HDF_Header, filename, "NumPart_ThisFile", H5T_NATIVE_UINT64, npart);


  //Ol = gizmo_old_readheader_float(HDF_Header, filename, "Omega_Lambda");
  //Om = gizmo_old_readheader_float(HDF_Header, filename, "Omega_Matter"); uint32_t npart_low[OGIZM_NTYPES], npart_high[OGIZM_NTYPES] = {0};
  //h0 = gizmo_old_readheader_float(HDF_Header, filename, "HubbleParam");  int64_t npart[OGIZM_NTYPES];
  //SCALE_NOW = gizmo_old_readheader_float(HDF_Header, filename, "Time");  float massTable[OGIZM_NTYPES];
  //BOX_SIZE = gizmo_old_readheader_float(HDF_Header, filename, "BoxSize");
  //BOX_SIZE *= OGIZM_LENGTH_CONVERSION;    gizmo_old_readheader_array(HDF_Header, filename, "NumPart_ThisFile", H5T_NATIVE_UINT64, npart);
  ...
  TOTAL_PARTICLES = ( ((int64_t)npart_high[OGIZM_DM_PARTTYPE]) << 32 ) 
    + (int64_t)npart_low[OGIZM_DM_PARTTYPE];
  
  H5Gclose(HDF_Header);
  // Check if the massTable entry for DM particles is zero
  if (massTable[OGIZM_DM_PARTTYPE] == 0.0f) {
      // Call the new function to read "Masses" dataset and update massTable
      gizmo_old_readmass_data(HDF_FileID, filename, massTable);
  
      // Now massTable[OGIZM_DM_PARTTYPE] should be updated with the correct mass
      if (massTable[OGIZM_DM_PARTTYPE] == 0.0f) {
  ...
  PARTICLE_MASS   = massTable[OGIZM_DM_PARTTYPE] * OGIZM_MASS_CONVERSION;
  ...
  printf("OGIZM: filename:       %s\n", filename);
  printf("OGIZM: box size:       %g Mpc/h\n", BOX_SIZE);
  printf("OGIZM: h0:             %g\n", h0);
  printf("OGIZM: scale factor:   %g\n", SCALE_NOW);
  printf("OGIZM: Total DM Part:  %" PRIu64 "\n", TOTAL_PARTICLES);
  printf("OGIZM: ThisFile DM Part: %" PRIu64 "\n", npart[OGIZM_DM_PARTTYPE]);
  printf("OGIZM: DM Part Mass:   %g Msun/h\n", PARTICLE_MASS);
  printf("OGIZM: avgPartSpacing: %g Mpc/h\n\n", AVG_PARTICLE_SPACING);
  
  if (!npart[OGIZM_DM_PARTTYPE]) {
    H5Fclose(HDF_FileID);
    printf("   SKIPPING FILE, PARTICLE COUNT ZERO.\n");
    return;
  }

  int64_t to_read = npart[OGIZM_DM_PARTTYPE];
  check_realloc_s(*p, ((*num_p)+to_read), sizeof(struct particle));

  // read IDs, pos, vel
  char buffer[100];
  snprintf(buffer, 100, "PartType%"PRId64, OGIZM_DM_PARTTYPE);
